...
- `join_channel({ name: "…", location: "local", watch: true })` - **channel-wide topic visibility.** Pass `location` explicitly whenever any remote is configured — `watch: true` with no location is **refused** (it does not default to local), so you cannot silently watch a brand-new empty local channel. Normally you only receive messages from topics you have explicitly joined. With `watch: true` you receive messages from **every** topic in the channel, including topics created after you subscribed, each tagged with its topic name. Use this when you are an **orchestrator watching a fleet**. Ordinary worker sessions should leave it off. Notes: read-only visibility (does **not** join you to the topics); **forward-only** (use `read_topic_messages` to backfill); **gap-aware**: a dropped stream replays what it can, and if the broker cannot you are TOLD — `whoami.eventStream.mayHaveMissedMessages: true` and a warning is pushed once per local subscribed channel (not "into the channel" as a single broadcast). **Local-transport only** for now (remote `watch` is refused — see KAI-413). `whoami` / `list_channels` report `watching`; `whoami` also reports `watchingActive` and `eventStream`.
...
| `whoami`                            | Show your name, objective, active channel, active topic, subscribed channels (with `watching` / `watchingActive`), `eventStream` health (`connected`, `mayHaveMissedMessages`), per-location transport status, and skill/server `versions`. |
