...
- `join_channel({ watch: true })` - **channel-wide topic visibility.** Normally you only receive messages from topics you have explicitly joined, which is what keeps you out of unrelated topic traffic. With `watch: true` you receive messages from **every** topic in the channel, including topics created after you subscribed, each tagged with its topic name. Use this when you are an **orchestrator watching a fleet**: without it, a topic you forget to join is one you are silently blind to. Ordinary worker sessions should leave it off. Notes: it is read-only visibility (it does **not** join you to the topics, so you are not a participant); it is **forward-only and not a durable log** (it does not backfill messages sent before you started watching, and the event stream has no replay, so anything published while it is reconnecting - >=2s after a broker restart - is missed; use `read_topic_messages` to catch up either way); it is **local-transport only** for now (a remote `watch` is refused rather than half-working - see KAI-413); and `whoami` / `list_channels` report `watching` (the watch you asked for) while `whoami` additionally reports `watchingActive` (whether it is in effect right now - `false` when the local event stream is disconnected, i.e. you are subscribed but currently deaf), so you can confirm you are actually seeing the channel.
