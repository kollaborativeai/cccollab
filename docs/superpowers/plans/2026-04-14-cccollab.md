...
describe('SocketModeListener', () => {
  ...
  it('handles human messages (no session prefix)', () => {
    ...
    expect(mockBus.push).toHaveBeenCalledWith({
      sender: 'human:U_HUMAN',
      text: 'hey team status?',
      ts: '111.777',
      channel: 'C123',
      threadTs: undefined,
    })
      ...
      conversations: {
        history: vi.fn().mockResolvedValue({
          ...
          messages: [
            ...
            {
              text: ':robot_face: *[stefan-dispatcher]* online | Role: fullstack | Status: Starting up',
              ts: '100.050',
            },
    ...
    webClient: {
      ...
      conversations: {
        history: vi.fn().mockResolvedValue({
          ok: true,
          messages: [
            { text: 'Recent msg 1', ts: '200.100' },
            { text: 'Recent msg 2', ts: '200.050' },
          ],
        }),
      },
    } as never,
...
describe('Channel Tools', () => {
  ...
  it('subscribe_channel returns history when read_history true', async () => {
    const result = await handleChannelTool(
      'subscribe_channel',
      { channel: 'team-alpha-collab', read_history: true },
      deps,
    )
...
describe('Conversation Tools', () => {
  ...
  it('start_conversation posts topic and returns thread_ts', async () => {
    const result = await handleConversationTool(
      'start_conversation',
      { channel: 'team-alpha-collab', topic: 'Auth refactor' },
      deps,
    )
    expect(deps.webClient.chat.postMessage).toHaveBeenCalledWith({
      channel: 'C123',
      text: expect.stringContaining('Auth refactor'),
    })
  ...
  it('join_conversation fetches history and announces', async () => {
    const result = await handleConversationTool(
      'join_conversation',
      { channel: 'team-alpha-collab', thread_ts: '300.100' },
      deps,
    )
  ...
  it('reply_in_conversation posts formatted reply', async () => {
    await handleConversationTool(
      ...
      { channel: 'team-alpha-collab', thread_ts: '300.100', text: 'Here is my review' },
      deps,
    )
    expect(deps.webClient.chat.postMessage).toHaveBeenCalledWith({
      channel: 'C123',
      thread_ts: '300.100',
      text: '*[stefan-dispatcher]*: Here is my review',
    })
  })
  ...
  it('list_conversations includes resolved when requested', async () => {
    const result = await handleConversationTool(
      'list_conversations',
      { channel: 'team-alpha-collab', include_resolved: true },
      deps,
    )
    expect(result).toContain('Auth refactor')
  ...
  it('resolve_conversation posts summary', async () => {
    await handleConversationTool(
      ...
      { channel: 'team-alpha-collab', thread_ts: '300.100', summary: 'Agreed on JWT approach' },
      deps,
    )
    expect(deps.webClient.chat.postMessage).toHaveBeenCalledWith({
      channel: 'C123',
      ...
      inputSchema: {
        ...
        properties: {
          ...
          include_resolved: {
            ...
            description: 'Include resolved conversations (default: false)',
          },
        },
        required: ['channel'],
      },
...
async function main() {
  ...
  const cleanup = () => {
    console.error('[slack-collab] Shutting down...')
    process.exit(0)
  }
...
main().catch((err) => {
  console.error('[slack-collab] Fatal error:', err)
  process.exit(1)
})
```
...
describe('Integration: end-to-end message flow', () => {
  ...
  it('full subscribe -> receive -> reply flow', async () => {
    ...
    expect(mockWeb.chat.postMessage).toHaveBeenCalledWith({
      channel: 'C123',
      thread_ts: '600.001',
      text: '*[stefan-dispatcher]*: Sure, on it',
    })
  })
