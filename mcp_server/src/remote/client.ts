 ...
 * without threading through the ConvexClient SDK internals, so the
 * refresh logic can be exercised with plain mocks.
 *
 ...
 *    `forceRefreshToken` is false → return cached token immediately.
...
export function makeClerkAuthFetcher(
  ...
  return async function clerkAuthFetcher({ forceRefreshToken }): Promise<string | null> {
    ...
    if (!forceRefreshToken && stillFresh) return currentAccessToken
