 ...
 *
 * Each row carries the org's `slug` when it has one (KAI-407) — the path
 * segment in `/{slug}/...` (not the full URL), and the preferred value to
 * pass to `introduce`. Orgs that never got a slug are addressable by `id`
 * only, so the key is omitted rather than emitted empty.
 */
